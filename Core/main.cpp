...
int main()
{
    ...
    bool bNeedCom = false;        //COM needed
    {
        ...
        szIndex = ftell(file);
        if (!(szIndex >= 7*sizeof(wchar_t) && szIndex <= 50*1024*1024 && szIndex%sizeof(wchar_t) == 0))        //[7 - "MVVSC\nF"]
        {
            fclose(file);
        ...
        //register hotkeys ------------------------------------------------------------------------
        for (szIndex = 0; szIndex < szCount; ++szIndex)
            RegisterHotKey(hWnd, szIndex, clsKeys[szIndex], clsKeys[szIndex+szCount]);
        clsKeys.fin();
    }
    ...
    STARTUPINFO si;
    pPi = &pi;        //***it's ok, as the program is completed after main
    pSi = &si;        //***it's ok, as the program is completed after main
...
#else
    PVOID pVoid;
    if (Wow64DisableWow64FsRedirection(&pVoid))
    {
        while (GetMessage(&msg, 0, 0, 0) > 0)
            DispatchMessage(&msg);
