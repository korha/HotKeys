...
BOOL WINAPI DllMain(HINSTANCE hInstDll, DWORD fdwReason, LPVOID)
{
    ...
    if (fdwReason == DLL_PROCESS_ATTACH)
    {
        if ((GetVersion() & 0xFF) == 5 &&        //Win2000/XP (NT5)
                (g_hFont = CreateFont(eFontHeight, 0, 0, 0, FW_BOLD, FALSE, FALSE, FALSE, ANSI_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, ANTIALIASED_QUALITY, DEFAULT_PITCH, L"Tahoma")))        //***it's ok, because the management window comes here
        {
            WNDCLASS wndCl;
            wndCl.style = 0;
            ...
            if (RegisterClass(&wndCl))        //***it's ok, because the management window comes here
            {
                ...
                {
                    if (SetLayeredWindowAttributes(g_hWnd, 0, 0, LWA_COLORKEY))        //***it's ok, because the management window comes here
                    {
                        bOk = true;
                        return TRUE;
                    }
                    DestroyWindow(g_hWnd);        //***it's ok, because the management window comes here
                }
                UnregisterClass(g_wGuidClass, wndCl.hInstance);        //***it's ok, because the management window comes here
            }
            DeleteObject(g_hFont);        //***it's ok, because the management window comes here
        }
    }
    else if (fdwReason == DLL_PROCESS_DETACH && bOk)
    {
        DestroyWindow(g_hWnd);        //***it's ok, because the management window comes here
        UnregisterClass(g_wGuidClass, hInstDll);        //***it's ok, because the management window comes here
        DeleteObject(g_hFont);        //***it's ok, because the management window comes here
    }
    return FALSE;
}
            ...
            if (mixerGetLineControls(hMixerObj, &mixerLineCtrls, MIXER_GETLINECONTROLSF_ONEBYTYPE) == MMSYSERR_NOERROR)
            {
                ...
                if (mixerSetControlDetails(hMixerObj, &mixerCtrlDtls, MIXER_SETCONTROLDETAILSF_VALUE) == MMSYSERR_NOERROR)
                {
                    ...
                    if (mixerGetLineControls(hMixerObj, &mixerLineCtrls, MIXER_GETLINECONTROLSF_ONEBYTYPE) == MMSYSERR_NOERROR)
                    {
                        ...
                        mixerCtrlDtls.paDetails = &mixerCtrlDtlsUns;        //***all right
                            ...
                            if (mixerGetControlDetails(hMixerObj, &mixerCtrlDtls, MIXER_GETCONTROLDETAILSF_VALUE) == MMSYSERR_NOERROR)
                            {
                                ...
                                if (mixerSetControlDetails(hMixerObj, &mixerCtrlDtls, MIXER_SETCONTROLDETAILSF_VALUE) == MMSYSERR_NOERROR)
                                {
                                    ...
                                    if (mixerGetLineControls(hMixerObj, &mixerLineCtrls, MIXER_GETLINECONTROLSF_ONEBYTYPE) == MMSYSERR_NOERROR)
                                    {
                                        ...
                                        mixerCtrlDtls.dwControlID = mixerCtrl.dwControlID;
                                        mixerCtrlDtls.cbDetails = sizeof(MIXERCONTROLDETAILS_BOOLEAN);
                                        mixerCtrlDtls.paDetails = &mixerCtrlDtlsBool;        //***all right
                                        if (mixerGetControlDetails(hMixerObj, &mixerCtrlDtls, MIXER_GETCONTROLDETAILSF_VALUE) == MMSYSERR_NOERROR)
                                        {
